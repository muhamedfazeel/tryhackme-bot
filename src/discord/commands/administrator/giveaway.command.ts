import { ActionRowBuilder, PermissionFlagsBits, SlashCommandBuilder, ChatInputCommandInteraction, ButtonBuilder, ButtonStyle, EmbedBuilder } from "discord.js";
import { COMMAND_CATEGORY, COMMAND_SCOPE } from "../../../shared/constants";
import { Giveaway } from "../../../schemas";
import { checkDate } from "../../../shared/utils/timer.util";
...
const GiveawayCommand: ICommand = {
    ...
    execute: async (interaction: ChatInputCommandInteraction, client: any) => {
        ...
        switch (subcommand) {
            case "create":
                ...
                const year: number = interaction.options.getInteger("year") as number;
                const time: number = interaction.options.getInteger("time") as number;
                const winners: number = interaction.options.getInteger("winners") as number;
                ...
                const giveawayMessage = await announcementChannel
                    ...
                    .catch((err: any) => {
                        logger.error("[GIVEAWAY] Error sending giveaway message:", err);
                        return null;
                    });
            ...
            case "stop":
                ...
                await endGiveaway(client, stopId, announce);
                logger.info(`Giveaway ended for giveaway ID: ${stopId}`);
            ...
            case "reroll":
                ...
                const amountToReroll = interaction.options.getInteger("amount") as number;
